...
const MOVIMIENTOS = [
  ...
  { fecha: "2026-06-29", activo: "GOOGL", tipo: "Compra", cantidad: 14.0, precio: 7732.3215, broker: "Balanz" }, // compra en dólares (cable/MEP): 6.2107 USD/u convertido a ARS ~1245
  { fecha: "2026-06-26", activo: "ADBE", tipo: "Venta", cantidad: 20.0, precio: 5732.6, broker: "Balanz" }, // venta en dólares (cable/MEP): 4.6045 USD/u convertido a ARS ~1245
  ...
  { fecha: "2024-08-13", activo: "AL30D", tipo: "Venta", cantidad: 10.0, precio: 612.291, broker: "IOL" },
...
function buildRealPortfolioHistory(holdings, historyCache) {
  ...
  const perTicker = uniqueTickers.map((h) => {
    ...
    if (hist && hist.length > 1) {
      ...
      return {
        valueAt: (date) => {
          // Para "hoy": si la caché diaria todavía no corrió, priceAt() cae
          // al último cierre cacheado (ayer) sin avisar -- eso hace que el
          // punto de "hoy" no coincida con el valor real en vivo, y el factor
          // de escala que fuerza esa coincidencia termina distorsionando
          // TODA la serie hacia atrás (el mismo problema que tuvimos con el
          // scaleFix de BTC). Para hoy exigimos coincidencia exacta de fecha
          // en la caché; si no está, usamos el precio en vivo real (h.price).
          const price = date === todayStr ? (historyMap[date] ?? h.price) : (priceAt(historyMap, sortedDates, date) ?? h.price);
          return qtyAt(date) * price;
        },
      };
    }
